...
class Node:
    ...
    def dfs(self, shift=0):
        if self.leaf:
            argmax = self.probs.keys()[0]
            for cl in self.probs:
                if self.probs[cl] > self.probs[argmax]:
                    argmax = cl
            return ' ' * shift + 'Predicted class: $%d\n' % argmax
        else:
            ret = ''
            ret += ' ' * shift + 'Nodes with feature #%d <= %d:\n' % (self.question[0], self.question[1])
            ret += self.left.dfs(shift + 4) 
            ret += ' ' * shift + 'Nodes with feature #%d > %d:\n' % (self.question[0], self.question[1])
            ret += self.right.dfs(shift + 4)
            return ret
